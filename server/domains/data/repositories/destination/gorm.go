...
func (g *gormProvider) Create(ctx context.Context, m *models.Destination) error {
	return err.ErrDuplicateValue(g.logs, g.GetModelName(), g.db.WithContext(ctx).Create(&m).Error)
}

// GetAll implements Repository.
func (g *gormProvider) GetAll(ctx context.Context, limit int64) (result models.SliceResult[models.Destination]) {
...
func (g *gormProvider) GetById(ctx context.Context, id uuid.UUID) (result models.SingleResult[*models.Destination]) {
...
func (g *gormProvider) GetByName(ctx context.Context, name string, limit int64) (result models.SliceResult[models.Destination]) {
	names := fmt.Sprint("%" + name + "%")
	result.Error = err.ErrRecordNotFound(g.logs, g.GetModelName(), g.db.WithContext(ctx).First(&result.Value, "id = ?", names).Error)
	return result
}
...
func (g *gormProvider) Update(ctx context.Context, m *models.Destination) error {
